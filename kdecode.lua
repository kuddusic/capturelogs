...
-- kudu start
--fields[0]=frame.time
--fields[0]=Field.new(rtsp)

local frametime = Field.new("frame.time")
-- our fake protocol
...
local exfield_time   = ProtoField.new("Extracted Time String", "extract.time", ftypes.STRING)
-- register the new fields into our fake protocol
exproto.fields = { exfield_string, exfield_hex, exfield_time}
    ...
    for i, field in ipairs(fields) do
        ...
        if #finfos > 0 then
            ...
            for _, finfo in ipairs(finfos) do
				...
				local frame_time= tostring(frametime() )
				local str1 = ftvbr:string(ENC_UTF_8)

                tree:add(exfield_string,string.gsub(str1,"\r\n",";" ) )
                tree:add(exfield_hex,tostring(ftvbr:bytes()))
				tree:add(exfield_time, string.sub(frame_time,1,28))
            end
        end
    end
